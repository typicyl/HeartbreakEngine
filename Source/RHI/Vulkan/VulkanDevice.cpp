...
struct PostUBO {
    ...
    // must stay byte-identical to PostCB and PostCommon.hlsli.
...
private:
    ...
    // 32 KB. Was 16 KB: PostUBO grew by the 80-byte brush-field block, and with a
    // 256-byte minUniformBufferOffsetAlignment that pushes the per-pass stride to the
    // next 512-byte step - halving how many passes fit. Running out is not a crash,
    // it silently reuses stale constants (constantStarvedPasses_), which is a
    // miserable bug to chase, so buy the headroom: 32 KB x frames-in-flight.
    static constexpr VkDeviceSize kPostArenaSize = 1u << 15; // 32 KB
